...
abstract contract DefaultBeforeAfterHooks is BaseHooks {
    ...
    function _setReserveValues(DefaultVars storage _defaultVars) internal {
        ...
        if (targetAsset != address(0)) {
            ...
            _defaultVars.scaledTotalSupply = IAToken(protocolTokens[targetAsset].aTokenAddress).scaledTotalSupply();
            _defaultVars.totalBorrow = IERC20(protocolTokens[targetAsset].variableDebtTokenAddress).totalSupply();
            ...
            _defaultVars.supplyCap = currentConfig.getSupplyCap() * 10 ** IERC20Detailed(targetAsset).decimals();
            _defaultVars.borrowCap = currentConfig.getBorrowCap() * 10 ** IERC20Detailed(targetAsset).decimals();
    ...
    function assert_BASE_GPOST_A() internal {
        for (uint256 i; i < baseAssets.length; i++) {
            assertLe(
                defaultVarsBefore.reseveNormalizedIncome[i], defaultVarsAfter.reseveNormalizedIncome[i], BASE_GPOST_A
            );
    ...
    function assert_BASE_GPOST_BCD() internal {
        for (uint256 i; i < baseAssets.length; i++) {
            if (defaultVarsBefore.reserveNormalizedVariableDebt[i] != defaultVarsAfter.reserveNormalizedVariableDebt[i])
            {
                assertTrue(
                    msg.sig == ILendingHandler.supply.selector || msg.sig == ILendingHandler.withdraw.selector
                        || msg.sig == IBorrowingHandler.borrow.selector || msg.sig == IBorrowingHandler.repay.selector
                        || msg.sig == ILiquidationHandler.liquidationCall.selector,
                    BASE_GPOST_B
                );
            }
            if (defaultVarsBefore.virtualUnderlyingBalance[i] != defaultVarsAfter.virtualUnderlyingBalance[i]) {
                assertTrue(
                    msg.sig == ILendingHandler.supply.selector || msg.sig == ILendingHandler.withdraw.selector
                        || msg.sig == IBorrowingHandler.borrow.selector || msg.sig == IBorrowingHandler.repay.selector
                        || msg.sig == ILiquidationHandler.liquidationCall.selector,
            ...
            if (defaultVarsBefore.reseveNormalizedIncome[i] != defaultVarsAfter.reseveNormalizedIncome[i]) {
                assertTrue(
                    msg.sig == ILendingHandler.supply.selector || msg.sig == ILendingHandler.withdraw.selector
                        || msg.sig == IBorrowingHandler.borrow.selector || msg.sig == IBorrowingHandler.repay.selector
                        || msg.sig == ILiquidationHandler.liquidationCall.selector,
    ...
    function assert_LENDING_GPOST_C() internal {
        ...
        uint256 totalSupplyUpdatedTreasury =
            _getRealTotalSupply(targetAsset, defaultVarsBefore.scaledTotalSupply, defaultVarsAfter.accruedToTreasury);
