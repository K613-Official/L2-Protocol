...
contract PoolRepayTests is TestnetProcedures {
    ...
    function test_repay_full_variable_borrow() public {
        ...
        emit BorrowLogic.Repay(tokenList.usdx, alice, alice, IERC20(address(varDebtUSDX)).balanceOf(alice), false);
        ...
        assertEq(
            contracts.poolProxy.getUserConfiguration(alice)
                .isBorrowing(contracts.poolProxy.getReserveData(tokenList.usdx).id),
            false
        );
    }
    ...
    function test_repayWithATokens_full_variable_borrow() public {
        ...
        vm.expectEmit(report.poolProxy);
        emit BorrowLogic.Repay(tokenList.usdx, alice, alice, IERC20(address(varDebtUSDX)).balanceOf(alice), true);
    ...
    function test_repayWithATokens_full_collateral_variable_borrow() public {
        ...
        vm.expectEmit(report.poolProxy);
        emit BorrowLogic.Repay(tokenList.usdx, alice, alice, IERC20(address(aUSDX)).balanceOf(alice), true);
        ...
        assertEq(
            contracts.poolProxy.getUserConfiguration(alice)
                .isUsingAsCollateral(contracts.poolProxy.getReserveData(tokenList.usdx).id),
            false
        );
    }

    function test_repayWithATokens_fuzz_collateral_variable_borrow(uint256 repayAmount, uint32 timeDelta) public {
        uint256 amount = 2000e6;
    ...
    function test_repayWithPermit(
        ...
        bytes32 digest = EIP712SigUtils.getTypedDataHash(
            permit, bytes(TestnetERC20(tokenList.usdx).name()), bytes("1"), tokenList.usdx
        );
        ...
        contracts.poolProxy.repayWithPermit(tokenList.usdx, repayAmount, 2, user, permit.deadline, v, r, s);
        vm.stopPrank();

        assertEq(usdx.balanceOf(user), underlyingBalanceBeforeRepayment - repayAmount);
        assertApproxEqAbs(IERC20(address(varDebtUSDX)).balanceOf(user), debtBalanceBefore - repayAmount, 1);
    }
    ...
    function test_partial_repay_borrow_variable_in_isolation() public {
        ...
        vm.expectEmit(report.poolProxy);
        emit IsolationModeTotalDebtUpdated(tokenList.wbtc, isolationModeTotalDebt - isolationDebtRepaid);
